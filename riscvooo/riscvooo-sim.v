  ...
  always @ ( * ) begin
    if ( !reset && ( status != 0 ) ) begin
      ...
      if ( verbose == 1'b1 ) begin
        ...
        $display( " ipc        = %f", ipc                        );
`ifdef BP_ENABLED
        $display( " branches   = %d", proc.ctrl.num_branches      );
        $display( " taken      = %d", proc.ctrl.num_taken         );
        $display( " resolved   = %d", proc.ctrl.num_pred_resolve  );
        $display( " correct    = %d", proc.ctrl.num_pred_correct  );
        $display( " mispredict = %d", proc.ctrl.num_mispredicts   );
`endif
      end
