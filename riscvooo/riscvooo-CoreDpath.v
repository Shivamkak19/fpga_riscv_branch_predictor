...
`ifdef BP_ENABLED
  input   [2:0] pc_mux_sel_Phl,        // 3-bit to fit pm_pred encoding
`else
...
`ifdef BP_ENABLED
  ,
  // Branch-predictor wiring between dpath and ctrl
  ...
  output        pred_target_match_Dhl  // 1 if F-stage prediction == D-stage JALR target
`endif
);
...
`ifdef BP_ENABLED
  // pm encoding (3-bit when predictor enabled):
  //   0=PC+4  1=branch redirect (mispredict)  2=jump (D)  3=jumpreg (D)  4=predicted target (F)
  // For pm=1 (branch), the redirect target is `branch_targ_Phl` if the
  // actual branch was taken, otherwise pc_Xhl+4 (predicted-taken corrected
  // back to fall-through). The selection between those two is gated by
  // redirect_to_target_Xhl coming from the ctrl module.
  wire [31:0] pcplus4_Xhl_w = pc_Xhl + 32'd4;
  wire [31:0] branch_redirect_Phl =
      redirect_to_target_Xhl ? branch_targ_Phl : pcplus4_Xhl_w;

  assign pc_mux_out_Phl
    = ( pc_mux_sel_Phl == 3'd0 ) ? pc_plus4_Phl
    : ( pc_mux_sel_Phl == 3'd1 ) ? branch_redirect_Phl
    : ( pc_mux_sel_Phl == 3'd2 ) ? jump_targ_Phl
    : ( pc_mux_sel_Phl == 3'd3 ) ? jumpreg_targ_Phl
    : ( pc_mux_sel_Phl == 3'd4 ) ? pred_target_Fhl
    :                              32'bx;
`else
  assign pc_mux_out_Phl
...
`ifdef BP_ENABLED
  //--------------------------------------------------------------------
  // Branch-predictor wiring out of dpath
  //--------------------------------------------------------------------
