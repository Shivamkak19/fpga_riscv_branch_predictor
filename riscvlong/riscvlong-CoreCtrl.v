...
`ifdef BP_ENABLED
  ...
  output        bp_correct_Xhl,
  output        mispredict_Xhl
`endif
);
...
`ifdef BP_ENABLED
  // pm_pred: F-stage predicted-taken redirect (B-type only)
  localparam pm_pred = 3'd4;
  assign pc_mux_sel_Phl
    = brj_taken_Xhl     ? pm_b               // mispredict redirect at X
    : brj_taken_Dhl     ? pc_mux_sel_Dhl     // jump redirect at D (JAL/JALR)
...
`ifdef BP_ENABLED
  //----------------------------------------------------------------------
  // F-stage Branch Pre-decode + Direction Predictor
  //----------------------------------------------------------------------
  // Pre-decode is combinational on the F-stage instruction (just the
  // imresp queue mux output) and the F-stage PC (latched in dpath, fed
  // through pc_Fhl input port).
  wire        is_branch_F;
  wire        is_jal_F;
  wire        is_jalr_F;
...
`ifdef BP_ENABLED
  ...
  assign bp_resolve_Xhl = ( inst_val_Xhl && (br_sel_Xhl != br_none) );
  assign bp_correct_Xhl = bp_resolve_Xhl && (actual_taken_Xhl == pred_taken_Xhl);
  assign mispredict_Xhl = bp_resolve_Xhl && (actual_taken_Xhl != pred_taken_Xhl);
  // X-stage redirect fires only on mispredict.
  wire brj_taken_Xhl = mispredict_Xhl;
  // When redirecting on mispredict, point at branch_targ if the branch
  // was actually taken, else at fall-through (pc_X + 4).
  assign redirect_to_target_Xhl = actual_taken_Xhl;
`else
...
//========================================================================
  ...
  // Branch-predictor diagnostic counters. Gated by the same
  // (stats_en || csr_stats) clause as num_cycles so they reflect the
  // same kernel/whole-program region the rest of the lab4 stats do.
  `ifdef BP_ENABLED
  reg [31:0] num_branches    = 32'b0;
  reg [31:0] num_taken       = 32'b0;
  reg [31:0] num_pred_resolve= 32'b0;
  reg [31:0] num_pred_correct= 32'b0;
  reg [31:0] num_mispredicts = 32'b0;
  `endif
...
`ifdef BP_ENABLED
        // Conditional-branch resolve in X (br_sel != br_none means the
        // X-stage instruction is a B-type)
        if ( inst_val_Xhl && (br_sel_Xhl != 3'd0) ) begin
          num_branches = num_branches + 1;
          if ( actual_taken_Xhl ) num_taken = num_taken + 1;
        end

        // Predictor weighed in?
        if ( bp_resolve_Xhl ) begin
          num_pred_resolve = num_pred_resolve + 1;
          if ( bp_correct_Xhl ) num_pred_correct = num_pred_correct + 1;
          else                  num_mispredicts  = num_mispredicts  + 1;
        end
`endif
